...
void merkle_open(uint64_t *auth_path, int leaf_idx, int proof_len, uint64_t* host_concatenated_tree) {
    ...
    while (current_index > 0) {
        ...
        // Copy the sibling hash from host_concatenated_tree to auth_path
        memcpy(auth_path + proof_len * CONCAT_WORDS, 
...
//     size_t codeword_layers = 3; //always 12 layers of codewords 
//     uint64_t codewords_combined[2 * field_words];  // Buffer to combine elements
//     uint64_t combined_hash_and_codeword[];
...
//         //store the sibling codeword (f_xx) in the proof path
//         proof_path[*proof_len] = (uint64_t *)malloc(field_words * sizeof(uint64_t));
//         memcpy(proof_path[*proof_len], codewords[layer][sibling_index], field_words * sizeof(uint64_t));
//         (*proof_len)++;

//         // Combine current codeword with sibling codeword for hash storage
//         memcpy(codewords_combined, codewords[layer][index], field_words * sizeof(uint64_t));
//         memcpy(codewords_combined + field_words, codewords[layer][sibling_index], field_words * sizeof(uint64_t));
...
//         SHA3(digest, (uint8_t *)combined_codewords, 2 * field_words * sizeof(uint64_t), 256); //first hash the combined codewords 
//         SHA3_host((uint8_t *)proof_path[*proof_len], (uint8_t *)codewords_combined, 2 * field_words * sizeof(uint64_t), 256);
//         (*proof_len)++;
//         //also store the combined codeword concatenated with this hash
...
int merkle_verify(uint64_t *root, size_t index, uint64_t *proof_path, size_t proof_len, uint64_t *leaf) {
    ...
    for (size_t i = 0; i < proof_len; ++i) {
        ...
        if (index % 2 == 0) {
            ...
            printf("tanjan reaching here\n");
            memcpy(combined, computed_hash, HASH_WORDS * sizeof(uint64_t));
            memcpy(combined + HASH_WORDS, proof_path + i * HASH_WORDS, HASH_WORDS * sizeof(uint64_t));
        } else {
            // Right node: concatenate proof path element with the computed hash
            printf("or reaching here\n");
            memcpy(combined, proof_path + i * HASH_WORDS, HASH_WORDS * sizeof(uint64_t));
