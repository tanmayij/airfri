...
// Adjust storage requirements for concatenated values and hashed values
...
__global__ void commit_kernel(
    ...
    uint64_t combined[FIELD_WORDS + FIELD_WORDS];
    ...
    if (I < N / 2 && N == 131072) { //hardcoded for now
        ...
        memcpy((uint8_t *)&device_layer_hashes[idx3], digest, 32);
    }
    __syncthreads();
...
void commit_launch(
    ...
    size_t tree_size = N * CONCAT_WORDS;
    uint64_t *device_concatenated_tree;
    cudaMalloc((void **)&device_concatenated_tree, tree_size * sizeof(uint64_t));
