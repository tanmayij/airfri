...
uint64_t* populate_eval_basis(int basis_len) {
    ...
    int count = 0;
    for (int i = 0; i < (preon.fri_domains[found].basis_len) * 5; i++) {
...
uint64_t get_offset_for_basis(int basis_len) {
    for (int i = 0; i < sizeof(preon.fri_domains) / sizeof(preon.fri_domains[0]); i++) {
        if (preon.fri_domains[i].basis_len == basis_len) {
            ...
            for (int j = 0; j < 5; j++) {  //Assuming the shift array has 4 elements
...
uint64_t* populate_all_bases(int *total_elements) {
    *total_elements = 0;
    ...
    for (size_t i = 0; i < sizeof(preon.fri_domains) / sizeof(preon.fri_domains[0]); i++) {
        for (int j = 0; j < (preon.fri_domains[i].basis_len) * 5; j++) {
...
int fri_num_rounds(Fri* fri) {
    ...
    int num_rounds = 0;
    int max_rounds = 16;  //set the maximum number of rounds to 16
...
void field_sample(uint8_t *byte_array, size_t byte_array_len, uint64_t *eval_basis, size_t basis_len, uint64_t *result) {
    uint64_t acc[4] = {0};  // 320 bits (5 * 64 bits)
    ...
    uint64_t combined_index = acc[0] ^ acc[1] ^ acc[2] ^ acc[3] ^ acc[4];
...
uint64_t ***commit_host(Fri *fri, uint64_t **codeword, int codeword_len, uint64_t **tree_layer)
{
    ...
    load_precomputed_inverses("fri_inverses_320.txt", precomputed_inverses);
    ...
    field_sample(alpha_bytes, 24, elements, 320, sampled_alpha);
...
size_t* prove(Fri* fri, uint64_t **codeword, uint64_t **tree_layer) {
    ...
    sample_indices(seed, 32, codeword_length/2, (fri->initial_domain_length >> (fri_num_rounds(fri)- 1)), fri->num_colinearity_tests, top_level_indices, reduced_indices);
    ...
    for (size_t i = 0; i < fri_num_rounds(fri) - 1; i++) { 
        query(fri, codewords, codewords[i], codeword_length, codewords[i + 1], codeword_length / 2, indices, i);
...
void test_fri(){
    ...
    uint64_t *poly_coeffs = (uint64_t *)malloc((initial_domain_length >> fri_num_rounds(fri)) * sizeof(uint64_t));
    if (poly_coeffs == NULL) {
        fprintf(stderr, "Failed to allocate memory for poly_coeffs\n");
        exit(1);
    }
