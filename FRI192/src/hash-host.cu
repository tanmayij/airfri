 ...
 * ---------------------------------------------------------------------- */
 ...
 sha3_return_t
 sha3_Init_host(void *priv, unsigned bitSize) {
 ...
 void
 sha3_Init256_host(void *priv)
 {
     sha3_Init_host(priv, 256);
 }
 
 void
 sha3_Init384_host(void *priv)
 {
     sha3_Init_host(priv, 384);
 }
 
 void
 sha3_Init512_host(void *priv)
 {
     sha3_Init_host(priv, 512);
 }
 
 enum SHA3_FLAGS
 sha3_SetFlags_host(void *priv, enum SHA3_FLAGS flags)
 {
     sha3_context *ctx = (sha3_context *) priv;
 ...
 void
 sha3_Update_host(void *priv, void const *bufIn, size_t len)
 {
     sha3_context *ctx = (sha3_context *) priv;
...
//  {
//      SHA3s_host(hm, (const uint8_t *[]){msg}, (size_t[]){msg_len}, 1, bitSize);
//  }
// void SHA3_host(uint8_t *hm, const uint8_t *msg, size_t msg_len, size_t bitSize) {

//     const uint8_t *msg[1] = { 0 };
//     size_t msg_len[1] = { msg_len };

//     // Debugging output
//     printf("SHA3_host Debug:\n");
//     printf("Message Length: %zu\n", msg_lens[0]);
//     printf("Message Bytes: ");
//     for (size_t i = 0; i < msg_lens[0]; i++) {
//         printf("%02x ", msg[0][i]);
//     }
//     printf("\n");

//     SHA3s_host(hm, msgs, msg_lens, 1, bitSize);

//     free(msg_copy); // Free the defensive copy
// }
void SHA3_host(uint8_t *hm, const uint8_t *msg, size_t msg_len, size_t bitSize)
{
    ...
    const uint8_t **msgArray = (const uint8_t **)malloc(sizeof(uint8_t *));
    size_t *msgLenArray = (size_t *)malloc(sizeof(size_t));

    if (!msgArray || !msgLenArray) {
        fprintf(stderr, "Memory allocation failed!\n");
        exit(1);
    }

    msgArray[0] = msg;
    msgLenArray[0] = msg_len;
    SHA3s_host(hm, msgArray, msgLenArray, 1, bitSize);
    free(msgArray);
    free(msgLenArray);
}
