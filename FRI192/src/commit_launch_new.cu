...
__global__ void merkle_kernel(
    ...
    if(I < N/2 && N == 32){ //only if N is 32, we use codewords
        ...
        int idx5 = I * (HASH_WORDS + FIELD_WORDS);
        //step 1
        memcpy(&device_combined_sibling_codewords[idx4], &device_combined_element[idx1], (FIELD_WORDS + HASH_WORDS) * sizeof(uint64_t));
        memcpy(&device_combined_sibling_codewords[idx4 + (FIELD_WORDS + HASH_WORDS)], &device_combined_element[idx2], (FIELD_WORDS + HASH_WORDS) * sizeof(uint64_t));
        //step 2
        ...
        //step 3 - concat hash with the current layer of codewords
        memcpy(&device_combined_element[idx5], &device_digest[idx3], (HASH_WORDS) * sizeof(uint64_t));
        memcpy(&device_combined_element[idx5 + (HASH_WORDS)], &device_codeword_nxt[idx3], (HASH_WORDS + FIELD_WORDS) * sizeof(uint64_t));
    }
...
void commit_launch(
    ...
    int initial_codeword_length, int final_codeword_length
) {
    printf("Starting commit_launch\n");
    printf("N = %d, FIELD_WORDS = %d\n", N, FIELD_WORDS);
    int basis_len = (int)log2(N);
    printf("basis len: %d\n", basis_len);
    //declare and allocate memory for flattened version of the codeword
    ...
    printf("\n");
    //now handle the merkle tree's current layer
    uint64_t *flattened_tree_layer;
    if(N == initial_codeword_length) {
    ...
    if (N == last_codeword_length * 2) { 
        int tree_idx = (int)log2(initial_codeword_length) - (int)log2(final_codeword_length);  // Start with layer 12 for N == 64
        int next_N = N / 2; 
        ...
        while (next_N >= 2) {
            ...
            if (next_N == 2) {
            cudaMemcpy(root, device_tree_layer + FIELD_WORDS, HASH_WORDS * sizeof(uint64_t), cudaMemcpyDeviceToHost); //pick HASH_WORDS size AFTER FIELD_WORDS because the order is FIELD_WORDS + HASHWORDS
